...
import (
	...
	uuidPkg "github.com/BangNopall/hology8-be/pkg/uuid"
)
...
func (s *userService) Register(ctx context.Context, user dto.UserRegister, referer string) error {
	...
	link := "http://elbccs-735956072.us-east-1.elb.amazonaws.com/" + "auth/verify-email/" + user.Email + "/" + emailVerPassword
...
func (s *userService) ForgotPassword(ctx context.Context, user dto.UserForgotPassword, referer string) error {
	...
	link := "http://elbccs-735956072.us-east-1.elb.amazonaws.com" + `/auth/reset-password/` + forgotPasswordToken
